...
// 0x00RRGGBB
#define RGB(r, g, b) \
    ((((unsigned)(r) & 0xFF) << 16) | (((unsigned)(g) & 0xFF) << 8) | ((unsigned)(b) & 0xFF))
...
// -------- disco color (integer HSV wheel: hue ∈ [0..1535])
...
static unsigned int hue_to_rgb(int h) {
    ...
    switch (seg) {
        case 0: r = 255; g = x;   b = 0;   break; // R → Y
        case 1: r = 255 - x; g = 255; b = 0;   break; // Y → G
        case 2: r = 0;   g = 255; b = x;   break; // G → C
        case 3: r = 0;   g = 255 - x; b = 255; break; // C → B
        case 4: r = x;   g = 0;   b = 255; break; // B → M
        default:r = 255; g = 0;   b = 255 - x;   break; // M → R
    }
    return RGB(r, g, b);
}
...
static void hud_print_static(void) {
    ...
    sys_write("  ← / →     switch shape (also , / .)\n");
    sys_write("  ↑ / ↓     faster / slower\n");
